  ...
  return (
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-lg overflow-hidden">
          {filtered.map(s => (
            <button key={s} onMouseDown={() => { onChange(s); setOpen(false); }}
              className="w-full text-left px-3 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700">{s}</button>
          ))}
        </div>
      )}
    </div>
      ...
      {steps.map((s, i) => (
        <div key={s.n} className="flex items-center">
          <div className="flex flex-col items-center gap-1">
            ...
            <span className={`text-xs font-medium hidden sm:block ${step === s.n ? 'text-purple-700 dark:text-purple-400' : 'text-zinc-400 dark:text-zinc-500'}`}>{s.label}</span>
          </div>
          {i < steps.length - 1 && (
            <div className={`h-0.5 w-12 sm:w-20 mx-1 mb-4 rounded-full transition-colors ${step > s.n ? 'bg-purple-700' : 'bg-zinc-200 dark:bg-zinc-700'}`} />
          )}
        </div>
      ))}
    </div>
...
function CheckingModeSelector({ value, onChange }: { value: CheckingMode; onChange: (m: CheckingMode) => void }) {
  return (
    <div>
      <p className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2">Checking Mode</p>
      <div className="flex gap-2">
        {CHECKING_MODES.map(m => (
          <button key={m} onClick={() => onChange(m)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors capitalize ${
              value === m ? 'bg-purple-700 text-white shadow-sm shadow-purple-700/20' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700'
            }`}>
            {m}
          </button>
        ))}
        ...
        <>
          <div className="animate-fade-in space-y-6">
            <Card className="p-6 space-y-4">
              <div>
                <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Exam Context</h2>
                <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">Define the exam term and class before selecting the answer key.</p>
              </div>
          ...
          <div className="animate-fade-in space-y-6">
            <Card className="p-6 space-y-5">
              <div className="flex items-center gap-3">
                <Icon name="book" className="w-6 h-6 text-purple-700 dark:text-purple-400" />
                <div>
                  <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Select Subject</h2>
                  <p className="text-xs text-zinc-400 dark:text-zinc-500">Choose a subject with a pre-configured answer key.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <button onClick={() => setSelectedSubjectId(s.id)} className="w-full text-left p-4">
                        <div className="flex items-start justify-between">
                          <p className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm">{s.name}</p>
                          {selected && <Icon name="check" className="w-4 h-4 text-purple-700 dark:text-purple-400 shrink-0 mt-0.5" strokeWidth={2.5} />}
                        </div>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{s.questions.length} Question{s.questions.length !== 1 ? 's' : ''}</p>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400">{total} Total Marks</p>
                      </button>
                      <div className={`flex divide-x text-xs border-t ${selected ? 'border-purple-200 dark:border-purple-800 divide-purple-200 dark:divide-purple-800' : 'border-zinc-100 dark:border-zinc-700 divide-zinc-100 dark:divide-zinc-700'}`}>
                        <button
                          onClick={() => startEdit(s)}
                          className="flex-1 flex items-center justify-center gap-1 py-1.5 text-zinc-500 dark:text-zinc-400 hover:text-purple-700 dark:hover:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1 py-1.5 text-zinc-500 dark:text-zinc-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                        >
                ...
                <button
                  ...
                  className="text-left rounded-2xl p-4 border-2 border-dashed border-zinc-300 dark:border-zinc-600 hover:border-purple-400 dark:hover:border-purple-600 flex flex-col items-center justify-center gap-1 bg-transparent transition-colors min-h-[90px]"
                >
                  <Icon name="plus" className="w-6 h-6 text-zinc-400 dark:text-zinc-500" />
                  <span className="text-sm text-zinc-500 dark:text-zinc-400">New Subject</span>
                </button>
              </div>
      ...
      {step === 2 && subjectMode === 'create' && (
        <>
          <div className="animate-fade-in space-y-4">
            <Card className="p-6 space-y-5">
              <div>
                <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">{editingSubjectId ? 'Edit Subject' : 'Create New Subject'}</h2>
                <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{editingSubjectId ? 'Update questions and expected answers for this subject.' : 'Add questions and expected answers. This subject will be saved for future use.'}</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Subject Name</label>
                <TextInput
                  ...
                  {showBankPanel && (
                    <div className="px-4 pb-4 space-y-3 border-t border-purple-100 dark:border-purple-800 bg-purple-50/40 dark:bg-purple-900/10">
                      <div className="relative pt-3">
                        <Icon name="search" className="absolute left-3 top-1/2 mt-1.5 w-3.5 h-3.5 text-zinc-400 pointer-events-none" />
                        <input
                          type="text"
                          value={chapterSearch}
                          onChange={e => setChapterSearch(e.target.value)}
                          placeholder="Search chapters…"
                          className="w-full pl-8 pr-3 py-1.5 text-sm border border-zinc-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                        />
                      </div>

                      <div className="space-y-1 max-h-48 overflow-y-auto">
                        {filteredChapters.map(c => (
                          <label key={c.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white dark:hover:bg-zinc-800 cursor-pointer">
                            <input
                              type="checkbox"
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-zinc-800 dark:text-zinc-200 truncate">{c.chapter}</p>
                              <p className="text-xs text-zinc-400 dark:text-zinc-500">{c.questions.length} questions</p>
                            </div>
                          </label>
                        ))}
                        {filteredChapters.length === 0 && (
                          <p className="text-xs text-zinc-400 dark:text-zinc-500 px-3 py-2">No chapters match "{chapterSearch}"</p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <input
                            ...
                            className="w-14 border border-zinc-300 dark:border-zinc-600 rounded-lg px-2 py-1.5 text-xs bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 focus:outline-none focus:ring-1 focus:ring-purple-600"
                          />
              ...
              <button onClick={addQuestion}
                className="w-full py-2.5 border-2 border-dashed border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-400 rounded-xl text-sm font-medium hover:border-purple-400 dark:hover:border-purple-600 hover:text-purple-700 dark:hover:text-purple-400 transition-colors">
                + Add Question
              </button>
      ...
      {step === 3 && (
        <>
          <div className="animate-fade-in space-y-6">
            <Card className="p-6 space-y-4">
              <div>
                <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Student Details</h2>
                <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">Enter the student's name and section for this grading session.</p>
              </div>
              ...
              {selectedSubject && (
                <div className="bg-zinc-50 dark:bg-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-x-4 gap-y-1">
                  <span><span className="font-medium text-zinc-700 dark:text-zinc-300">Term:</span> {examTerm}</span>
                  <span><span className="font-medium text-zinc-700 dark:text-zinc-300">Class:</span> {examClass}</span>
                  <span><span className="font-medium text-zinc-700 dark:text-zinc-300">Subject:</span> {selectedSubject.name}</span>
                  <span><span className="font-medium text-zinc-700 dark:text-zinc-300">Questions:</span> {selectedSubject.questions.length}</span>
                  <span><span className="font-medium text-zinc-700 dark:text-zinc-300">Mode:</span> {checkingMode.charAt(0).toUpperCase() + checkingMode.slice(1)}</span>
                </div>
              )}

              <div>
                <button onClick={() => setShowAdvanced(p => !p)}
                  className="flex items-center gap-2 text-xs text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300">
                ...
                {showAdvanced && (
                  <div className="mt-3">
                    <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
                      ...
                      <span className="ml-2 text-xs font-normal text-zinc-400">(optional — semantic similarity fallback)</span>
                    </label>
                    <TextInput
                      type="password" value={hfApiKey} placeholder="hf_…"
                      onChange={e => dispatch({ type: 'SET_HF_API_KEY', payload: e.target.value })}
                    />
                    <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-500">Without this, keyword overlap is used as fallback.</p>
                  </div>
                )}
              </div>
            </Card>
