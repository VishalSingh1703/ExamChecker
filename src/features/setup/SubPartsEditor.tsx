    ...
    <div className="space-y-2 mt-2">
      {subparts.map(sp => (
        <div key={sp.id} className="flex items-start gap-2">
          <span className="text-xs font-bold text-ink-500 dark:text-ink-400 mt-2.5 w-4 shrink-0">{sp.label}.</span>
          <textarea
            value={sp.question}
            onChange={e => updateSubpart(sp.id, e.target.value)}
            placeholder={`Sub-part ${sp.label}…`}
            rows={2}
            className="flex-1 border border-ink-200 dark:border-ink-700 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-600 resize-none bg-white dark:bg-ink-800 text-ink-800 dark:text-ink-200 placeholder-ink-300 dark:placeholder-ink-600"
          />
          <button
            onClick={() => removeSubpart(sp.id)}
            className="mt-2 text-ink-300 dark:text-ink-600 hover:text-red-500 transition-colors"
      ...
      {diagram && (
        <div className="relative inline-block">
          <img src={diagram} alt="Diagram" className="max-h-32 rounded-xl border border-ink-200 dark:border-ink-700 object-contain" />
          <button
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={addSubpart}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-ink-100 dark:bg-ink-800 text-ink-600 dark:text-ink-400 text-xs font-medium hover:bg-ink-200 dark:hover:bg-ink-700 transition-colors border border-ink-200 dark:border-ink-700"
        >
