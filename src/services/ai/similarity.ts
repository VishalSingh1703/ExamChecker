/**
 * Standalone answer-similarity scoring, used by HistoryView's Update modal.
 * Chain: Gemini (mark-scheme grading) → HuggingFace sentence similarity →
 * keyword-weighted Jaccard overlap.
 */
import type { SimilarityResult } from '../../types';
import { env, resolveGeminiKey } from '../../config/env';
...
// ── Gemini mark-scheme scoring (primary) ──────────────────────────────────────

async function getGeminiScore(extracted: string, expected: string, apiKey: string, keywords: string[], marks: number): Promise<number> {
...
Respond with ONLY a single decimal number between 0.0 and 1.0. No explanation, no other text.`;
  ...
  const text = await callGemini(apiKey, [{ text: prompt }], { maxOutputTokens: 200 });
  const match = text.match(/\d+(\.\d+)?/);
  if (!match) throw new Error(`AI returned non-numeric: "${text.slice(0, 50)}"`);
  return Math.max(0, Math.min(1, parseFloat(match[0])));
}

// ── HuggingFace sentence similarity (secondary) ───────────────────────────────
...
export async function getSemanticSimilarity(
  ...
  const geminiKey = resolveGeminiKey(geminiApiKey);
  const hfKey = hfApiKey?.trim() || env.hfApiKey;
